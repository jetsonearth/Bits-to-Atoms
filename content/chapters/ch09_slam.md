...
<figure>
  <img src="/images/ch09/loop_closure.gif" alt="LIO-SAM 回环检测与位姿图优化过程" />
  <figcaption>回环检测（Loop Closure）可视化 — 当机器人重新经过已访问区域时，系统检测到回环并触发全局位姿图优化，累积漂移被一次性校正，地图精度显著提升。图源：LIO-SAM / GitHub</figcaption>
</figure>
...
<figure>
  <img src="/images/ch09/slam_mapping.gif" alt="slam_toolbox 实时建图过程：机器人边移动边构建 2D 占用栅格图" />
  <figcaption>slam_toolbox 实时建图演示 — 机器人在环境中移动，LiDAR 扫描数据逐步拼接成完整的 2D 占用栅格图（白色为可通行区域，黑色为障碍物，灰色为未探索区域）。图源：slam_toolbox / GitHub</figcaption>
</figure>
...
<figure>
  <img src="/images/ch09/lio_sam_demo.gif" alt="LIO-SAM 3D 点云建图：LiDAR + IMU 紧耦合生成高精度三维地图" />
  <figcaption>LIO-SAM 实时 3D 点云建图 — 融合 LiDAR 点云与 IMU 惯性数据，在 RViz 中逐步构建高精度的三维环境地图。图源：LIO-SAM / GitHub</figcaption>
</figure>
...
<figure>
  <img src="/images/ch09/slam_occupancy_grid.png" alt="RViz 中的 2D 占用栅格图：SLAM 建图的典型输出" />
  <figcaption>RViz 中的 2D 占用栅格图 — SLAM 建图的典型输出，黑色线条表示墙壁等障碍物，浅灰色区域为可通行空间，深灰色为未探索区域。这张地图将直接交给 Nav2 做路径规划。图源：MOGI-ROS / GitHub</figcaption>
</figure>
