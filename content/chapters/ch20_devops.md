...
如果 ROS 2 是机器人的"操作系统"，那 **colcon** 就是它的构建工具。类似于 Web 世界里 npm/yarn 之于 Node.js，或者 cargo 之于 Rust - 但 colcon 要管理的东西更杂，因为 ROS 2 的包可以是 C++、Python 或者混合的。

先理解 ROS 2 的 **workspace**（工作空间）结构：

```
my_robot_ws/                  # workspace 根目录
├── src/                      # 所有源码包放这里
│   ├── my_slam/              # 一个 ROS 2 package
│   │   ├── CMakeLists.txt    # C++ 包的构建配置
...
# 只编译某个包
colcon build --packages-select my_navigation

# 编译某个包和它所有的依赖
colcon build --packages-up-to my_navigation

# 编译完后 source 一下才能使用
source install/setup.bash
```

看起来简单，但 colcon 构建中的坑之多，足以让每个 ROS 2 新手怀疑人生。

**经典坑 1：忘了 source**。编译完之后必须 `source install/setup.bash`，否则系统找不到你刚编译好的包。每开一个新终端都要 source 一次。绝大多数 ROS 2 开发者都会在 `~/.bashrc` 里加一行自动 source，但如果你同时开发多个 workspace，自动 source 反而会导致混乱 - 因为你可能在 workspace A 的终端里 source 了 workspace B 的环境。

**经典坑 2：编译顺序**。colcon 会根据 `package.xml` 里声明的依赖关系自动决定编译顺序。但如果你声明漏了一个依赖，可能第一次编译能过（因为恰好按正确顺序编译了），换一台干净机器就炸了。`--packages-up-to` 可以帮你验证依赖声明是否完整。

**经典坑 3：自定义消息（msg/srv）的编译**。如果你定义了自定义消息类型（第 8 章 DDS 那章提过），消息定义包必须先编译完，使用这些消息的包才能编译。colcon 理论上能自动处理这个顺序，但实践中经常因为缓存问题导致"找不到消息类型"的错误。遇到这种情况，`colcon build --cmake-clean-cache` 清掉缓存重新来一遍。

**经典坑 4：C++ 和 Python 混合包**。一个包里既有 C++ 节点又有 Python 节点，CMakeLists.txt 的写法容易出错。新手建议把 C++ 和 Python 拆成两个独立的包，通过 topic/service 通信，等熟悉了再考虑混合包。

还有一个影响开发体验的细节：C++ 包每次改完代码都要重新编译，而 Python 包如果用 `colcon build --symlink-install`，改了 Python 代码不需要重新 build，因为 install 目录里的文件只是指向 src 的符号链接。这个参数建议开发阶段默认开启。
